...
def generate_launch_description():
    # Get URDF
    my_gazebo_world_flie = os.path.join(get_package_share_diretory('gpg_gazebo', 'building_my_robot.sdf')

    gz args = LaunchConfiguration('gz args', default="-r "+ my_gazebo_world_flie)
    urdf = os.path.join(get_package_share_directory('gpg_remote'), 'gopigo3.urdf')
    
    sim_publisher = Node(package="ros_gz_sim",
                        executable="gz_sim",
                        name="gz_sim",
                        output="screen",
                        parameters=[{"gz args": gz args}])
